...
class Hard2Scene(AbstractScene):
    def __init__(self, screen_height, screen_width) -> object:
        ...
        self.gap = 250  # pixels
        self.tfrequency = 450  # ms, how frequent i want trees
    ...
    def render(self, screen):
        ...
        if len(self.tree_group) > 0:
            #Checks to see if x coordinate of the left monkey is greater than x coordiante of the right tree to see if its passed
        ...
        if self.game_over == False and self.swinging == True:
            ...
            if current_time - self.last_tree > self.tfrequency:
                ...
                btm_tree = Tree(self.screen_width, int(self.screen_height / 2) + tree_height, -1, 150)
                top_tree = Tree(self.screen_width, int(self.screen_height / 2) + tree_height, 1, 150)
